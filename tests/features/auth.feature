...
Функционал: Регистрация и аутентификация пользователя
  ...
  Сценарий: удаление юзера из группы
    Допустим я залогинен как пользователь "johndoe@test.com"
    И попадаю на страницу редактирования профиля у залогиненного пользователя
    Когда редактирую данные в форме профиля
    И нажимаю нажимаю на кнопку Update profile
    То вижу "Update profile successful"