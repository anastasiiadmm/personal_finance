...
const Categories = () => {
  ...
  return (
    <>
      <GridContainer>
        <GridItem xs={12}>
          <Button onClick={()=>dispatch(historyPush('/category/add'))} block variant="contained" color="primary">Add new category</Button>
        </GridItem>
        <GridItem xs={12}>
        ...
        <Fade in={open}>
          <div className={classes.paper}>
            <Paper>
              ...
              <Grid>There are {categoryToDelete && categoryToDelete.transactions.length} transactions in this
                category.</Grid>
              <form onSubmit={handleSubmit}>
                <FormControl component="fieldset">
                  <FormLabel component="legend">Gender</FormLabel>
                  <RadioGroup defaultValue={cascadeDelete} aria-label="gender" name="gender1" value={cascadeDelete}
                              onChange={handleChange}>
                    <FormControlLabel value={'cascade'} control={<Radio/>}
                                      label="Delete transactions along with category"/>
                    <FormControlLabel value={'notcascade'} control={<Radio/>}
                                      label="Assign a different category to transactions (select from the list)"/>
                  </RadioGroup>
                </FormControl>
                <Grid item xs>
                  <ButtonWithProgress
        ...
        <Fade in={openListToDelete}>
          <div className={classes.paper}>
            {categories && categories.map(category => {
              return <Box mb={1} key={category.id}
                          onClick={() => handleDeleteCategoryAndChangeTransactions(category.id)}>
